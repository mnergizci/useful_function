...
University of Leeds, Gamma Remote Sensing
...
if len(sys.argv) < 3:
    print('Please provide frame and pair information: i.e python subswath_bovl_mn_testing.py 021D_05266_252525 20230129_20230210')
    sys.exit(1)
...
# Suppress all output by redirecting stdout and stderr to /dev/null
...
##create the path for SLC1_tab, this is necesseary to run the code everywhere in terminal not only in $batchdir.
...
print(f'widht={width}, lenght={az_line} of multilooked image')
...
try:
  subprocess.run(exec_str, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
for prefix in ['coh', 'scaled', 'unscaled']: 
    ...
    try:
      ...
      print(f"An error occurred while executing the command: {e}")
    # dest_file=os.path.join(GEOC_folder,pair,f'{pair}_diff_double_mask_coh.geo.tif')
    # shutil.move(geoc_tif, dest_file)
    # print(BLUE + f'please run for quick preview: create_preview_wrapped {geoc_tif}'+ENDC)

# Now, restore stdout and stderr for the final print
